...
while cikl == True:
    ...
    if keys[pygame.K_o]:
        ...
        root.title(money)
        root.config(bg='black')
        money1 = Button(root, text=f'this a money {money}', font='10', bg='black', width='20', height='20', fg='yellow', command=money2)
        money1.pack()
...
# Это Диалог с волшебником у тебя здесь написано если игрок нажмет на т в конкретных кординатах то тогда и будет роботат твой скрыпт если нет то он не будет включатся
    if keys[pygame.K_t] and playerx + speed > 1300 :
        ...
        if talk == 2:
            ...
            root.title('lol')
            dialog_1_lvl_5 = Label(root, text='well done now kill the monsters behind you')
            dialog_1_lvl_5.pack()
